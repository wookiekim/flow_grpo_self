...
def general_ocr_sd3_8gpu():
    ...
    config.save_freq = 60 # epoch
    config.eval_freq = 60
    config.save_dir = 'logs/ocr/sd3.5-M_self_online_noCFGforSDS_0.5steps_10steps_CONT_CONT'
    config.reward_fn = {
        "ocr": 1.0,
    }
...
def general_ocr_flux_8gpu():
    ...
    config.resolution = 512
    config.sample.train_batch_size = 8 #3
    config.sample.num_image_per_prompt = 16 #24
    ...
    config.eval_freq = 10
    config.save_dir = 'logs/pickscore/flux-group24-8gpu-tryimprove-batch8-num16-0.6timestep_PROPER'
    config.reward_fn = {
        "ocr": 1.0,
    }
